...
const sortByChildren = (charArray) => {
  // Solution code here...
  let childrenNum = charArray.sort((a, b) => {
...
const citiesAtoJ = (arr) => {
  // Solution code here...
  let cities = arr.filter(item => {
    const regex = /(^[A-J])\w+/;
    let test = item.match(regex);
    if (test) {
      return test;
    }
  });
