...
const addPurchases = (arr) => {
  // Solution code here...
  let purchase = arr.reduce ((acc,val) => {
...
const returnNames = (arr) => {
  // Solution code here...
  let starWar = arr.reduce ((acc,val) => {
    acc.push(val.name);
    return acc;
  },[])
...
const reversedString = (str) => {
  // Solution code here...
  // but I tried also so solve it with .reverse() or .join("") but it did not work.
  let reverse = str.split("");
  let array = reverse.reduce ((acc,val) => {
    val = val + acc
    return val;
  },'')
  return array;
