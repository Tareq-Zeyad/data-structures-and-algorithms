...
def test_dfs_with_root_Pandora(dfs):
    graph = dfs[0]
    pandora = dfs[1]
    assert graph.dfs(pandora) == [
        'Pandora', 'Metroville', 'Naboo', 'Narnia', 'Monstroplolis', 'Arendelle']


def test_dfs_with_root_Metroville(dfs):
    graph = dfs[0]
    narnia = dfs[3]
    assert graph.dfs(narnia) == ['Metroville', 'Pandora',
                                 'Naboo', 'Narnia', 'Monstroplolis', 'Arendelle']
